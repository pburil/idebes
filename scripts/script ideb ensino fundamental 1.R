...
cor.test(base_final_teste$ideb, base_final_teste$`Indicador Adoção Aluno`)
...
ggplot(base_final_teste, aes('Indicador Adoção Aluno', IDEB)) +
  geom_jitter()+
  theme_minimal()

ggplot(base_final_teste, aes('Indicador Adoção Aluno', 'Indicador Engajamento Aluno', color = IDEB)) +
  geom_jitter(size = 4)+
  theme_minimal() 
...
regressao <- lm(base_final_teste$IDEB ~ base_final_teste$`Indicador Adoção Aluno`, data = base_final_teste)
...
regressao2 <- lm(base_final_teste$IDEB ~ base_final_teste$`Indicador Engajamento Aluno` + base_final_teste$`Indicador Adoção Aluno`, data = base_final_teste)
summary(regressao2)
...
ggplot(base_final_outliers, aes(base_final_outliers$'Indicador Adoção Aluno', fill = base_final_outliers$'IDEB Divulgado')) +
  geom_density(alpha = 0.3)
...
regressao4 <- lm(base_outliers_filtrada$IDEB ~ base_outliers_filtrada$`Indicador Engajamento Aluno` + base_outliers_filtrada$`Indicador Adoção Aluno` + 
